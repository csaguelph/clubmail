...
import { Building2, Calendar, Clock, Mail, Users } from "lucide-react";
...
interface AllScheduledCampaignsCalendarProps {
  campaigns: ScheduledCampaign[];
  isLoading?: boolean;
}

export default function AllScheduledCampaignsCalendar({
  campaigns,
  isLoading,
}: AllScheduledCampaignsCalendarProps) {
  if (isLoading) {
    return (
    ...
    return (
      <div className="rounded-lg border border-gray-200 bg-white p-12 text-center">
        ...
        <p className="text-gray-600">
          There are no campaigns scheduled for delivery across the platform.
        </p>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-4">
          ...
          <p className="text-2xl font-bold text-gray-900">
            {campaigns.reduce((sum, c) => sum + (c._count.emails || 0), 0)}
          </p>
        </div>
        <div className="rounded-lg border border-gray-200 bg-white p-4">
            ...
            <div className="space-y-3">
              {dateCampaigns
                ...
                .map((campaign) => (
                  <Link
                    key={campaign.id}
                    href={`/clubs/${campaign.club.slug}/campaigns/${campaign.id}`}
